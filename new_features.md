...
- <mark>[ ] [changes] **[DS-102] Release Management Workflow UX + Progress Guidance**</mark>
    Why: The current release flow lacks a clear, stepwise UI and consistent visual structure, which increases confusion and slows releases.
    What: Redesign the Release Management workflow UI with numbered, gated steps and a consistent layout. Step rows must render in this order: status symbol → step number → instructions → data field (if required) → action button (if required). Status symbols: empty circle = not started; blue dot = in progress; green circled check = completed. Virtual columns are vertically aligned with small spacing between steps. A new step becomes visible only after the previous step completes successfully. Add a Progress Status canvas with two tiles: (A) Current Progress Status showing the active status message (e.g., “no local changes detected”), always includes a positive confirmation statement on success, and shows red text with clear fix instructions when something fails; (B) System Status tile as already implemented (Repo path, Origin, etc.) with all current fields preserved. Errors must be explicit and actionable; no silent failures.
